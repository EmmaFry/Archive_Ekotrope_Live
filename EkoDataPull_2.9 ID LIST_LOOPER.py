/******************************
EkoDataPull_2.9 ID LIST_LOOPER.py 
unfinished/scrapped version of ekotrope big super looper
Exports to -> eko_log_list_loop.csv
...
def main(projectid):
    ...
    url = "http://app.ekotrope.com/api/v1/projects?status=UNREGISTERED"
    ...
    print("\tOnly pulling projects %s.\n" % projectid)
    lastWeek_df = ETcsv[ETcsv["id"].isin(projectid)]
    ...
    if numProjects == "0":
        print("All projects are up to date! Exiting program.")
        exit()
...
# 
# 
# ---this is blank space to make the rows line up with the non-specific ID 
# ---version to make debugging easier
# 
# 
# 
# 
...
#------------------------------------------------------------------------   
    ...
    for index,row in projectMasterDF.iterrows():
        try:   
           ...
           with open("ekotropr_log.csv", "a") as log:
...
def eko_list_looper(myCSV):
    data = pd.read_csv(myCSV)
    projectID_list = data['ekotropeid'].tolist()
    chunk_size= 4
    print(len(projectID_list))
    for i in range(0, len(projectID_list), chunk_size):
        chunk = projectID_list[i:i+chunk_size]
        print(chunk)
        try:
            print('attempting:' + str(chunk))
            main(chunk)
            with open("eko_log_list_loop.csv", "a") as log:
                    log.write(str(chunk)+', success,'+ str(dt.datetime.now()) +'\n')
            print('succeeded: ' +  str(chunk))    
        except Exception as e:
            with open("eko_log_list_loop.csv", "a") as log:
                    log.write(str(chunk)+','+ str(dt.datetime.now()) +'\n')
                    log.write(logging.Formatter('[%(asctime)s] p%(process)s {%(pathname)s:%(lineno)d} %(funcName)s %(levelname)s - %(message)s','%m-%d %H:%M:%S')+'\n')
            print('failed: ' + str(e) + str(chunk))
        
        # process chunk of size <= chunk_size


eko_list_looper(
'temporaryEkotrope.csv'
)
