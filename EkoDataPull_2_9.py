/*********************
the older version of EkoDataPull_2.9_BIG_SUPER_LOOP.py
*******************/
...
urlString = 'http://pshmn.com/ppLnYNe'
...
def main():
    ...
    url = "http://api.ekotrope.com/api/v1/projects?status=SUBMITTED_TO_REGISTRY"
    ...
    print("\tNarrowing scope of data to the last 2 days...")
    range_max = ETcsv["selfOrPlanLastSavedAt"].max()
    range_max = dt.datetime.now(dt.timezone.utc) +dt.timedelta(days=1)
    range_min = range_max - dt.timedelta(days=2)
    ...
    for index,row in lastWeek_df.iterrows():
        ...
        url = "http://api.ekotrope.com/api/v1/projects/%s" % currentID
    ...
    for index,row in projectMasterDF.iterrows():
        try:   
            ...
            url = "http://api.ekotrope.com/api/v1/houseplans/%s" % currentID
            res = requests.request("GET", url, data=payload, headers=headers).json()
            res = str(res)
            print(type(res))
           ...
           with open("ekotrope_log.csv", "a") as log:
    ...
    # housePlan2["id"] = housePlan1["id"]
    housePlan2.insert(loc=0, column="id", value=housePlan1["id"]) # Swapped due to CopyError and this is more predictable.
...
try:
    main() #parameter is days back in time to look
    handle = urllib.request.urlopen(urlString)
    handle.read()
    handle.close()

except Exception as e:
        with open("EkoDataPull_2_9_Log.csv", "a") as log:
                 log.write('EkoDataPull_2_9.csv,'+ str(e) + ',' + str(dt.datetime.now()) +'\n')
