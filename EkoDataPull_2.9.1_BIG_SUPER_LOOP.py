/*******************************
the old version of EkoDataPull_2.9_BIG_SUPER_LOOP.py that we were using to experiment with, but did not delete
*************************/
    ...
    while start_day > -1:
        ...
        try:
            ...
            with open("LOG_ekotrope_super_looper_log.csv", "a") as log:
                ...
                with open("LOG_ekotrope_super_looper_log.csv", "a") as log:
                         log.write(str(start_day)+','+ str(end_day)+','+str(interval)+','+str(e) +','+ str(dt.datetime.now()) +'\n')
